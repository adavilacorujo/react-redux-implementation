...
const columns = [
  ...
  { field: 'exercise', headerName: 'Workout', width: 200, renderCell: (params) =>  {
    return (
        <Tooltip title={params?.row.exercise} >
            <span className="table-cell-trucate">{params?.row.exercise}</span>
        </Tooltip>)
  }},
  { field: 'instructions', headerName: 'Instructions', width: 280, editable: true, renderCell: (params) =>  {
    return (
