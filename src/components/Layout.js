...
import {
  ...
  removeMuscleFilter,
  selectMuscleText,
} from "./app/searchMuscleTextSlice";
...
export default function Layout({ children }) {
  ...
  const [renderCommands, setRenderCommands] = useState(false);
    ...
    <Box sx={{ display: "flex" }}>
      ...
      >
        ...
        <Box sx={{ overflow: "auto" }}>
          ...
          />
          <MuscleSelector setRenderCommands={setRenderCommands} />
          {renderCommands && <CommandSelector />}
          <List>
            {menuItems.map((text, index) => (
              <ListItem
                key={text}
                disablePadding
                onClick={() => handleMuscleClick(text)}
                sx={
                  muscleText.includes(text.toLowerCase())
